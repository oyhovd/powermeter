...
void adv_update(void)
{
	...
	if((s32_t)(now - last_time) > M_ADV_UPDATE_TIME_MS)
	{
		...
		uint32_t size = snprintf(name, sizeof(name), "kW %d.%03d %d.%03d %d.%03d", last_min/1000, last_min%1000, last_twenty/1000, last_twenty%1000, last_hour/1000, last_hour%1000);
